...
def subset_file(year, output_dir, nthread):
    ...
    grid_check = pd.DataFrame([grid_ids[(grid_ids['lon'] == lon) & (grid_ids['lat'] == lat)].iloc[0] for lon, lat in grid_lonlats]).sort_values('id')
    grid_check['id'] = grid_ids['id'].iloc[0] + grid_check.index.values
    grid_check['huc2'] = grid_check['huc2'].astype(int).astype(str).str.zfill(2)
    grid_check['id'] = grid_check['id'].astype(int)
    grid_check.to_csv(input_csv.replace('.csv', '_check.csv'), index = False)

    list_outputs, list_paths = [], []
    for _, row in grid_check.iterrows():
        lon, lat = row.lon, row.lat
        huc2_code = int(row['huc2'])
        point_id = int(row['id'])
