...
def run_calibration(path_csv, point_id, path_output):
  ...
  if not os.path.islink(f'{subpath_output}/input_symln'): os.symlink(input_path, f'{subpath_output}/input_symln')

  # aggregate forcing files into 6 hours and save in the same input folder, "forcings_6h_16thdeg_*.nc"
  # concatenate runoff files into one single file not to repeat during calibrations (potentially to increase efficiency), "runoff_concat_16thdeg_*.nc"
  def update_forcing_runoff(input_path):
    files_forcing = glob.glob(f'{input_path}/forcings_16thdeg_{id_ll}_*.nc'); files_forcing.sort()
    ...
    files_runoff = glob.glob(f'{input_path}/runoff_16thdeg_{id_ll}_*.nc'); files_runoff.sort()
  ...
  with xr.open_dataset(f'{subpath_output}/params.nc') as ds:
    for s in ['infilt', 'Dsmax', 'Ds', 'Ws', 'depth2', 'depth3', 'expt2', 'expt3']:
      ...
      else: p.append(float(ds[s].isel(lon = 0, lat = 0)))
  ...
  for i, arr in enumerate(plim):
    if isinstance(arr[0], str) and arr[0].endswith('%'): plim[i][0] = p[i] * (1 + int(arr[0][:-1]) / 100)
    if isinstance(arr[1], str) and arr[1].endswith('%'): plim[i][1] = p[i] * (1 + int(arr[1][:-1]) / 100)
...
'''
  ...
  if os.path.isdir('nocalib'): shutil.rmtree('nocalib')
  os.makedirs('nocalib')
  shutil.copy('params.nc', './nocalib/params_updated.nc')
  shutil.copy('config.txt', './nocalib/')
  shutil.copy('vic_image.exe', './nocalib/') # vic_image
  ...
  if os.path.isdir('mod0'): shutil.rmtree('mod0')
  #os.system('./Ostrich')
  subprocess.run(['./Ostrich', '>', '/dev/null'])
  ...
  shutil.copytree('mod0', 'mod0_calib')


if __name__ == '__main__':
  if len(sys.argv[2].split(',')) == 1:
    path_csv, idx, path_output = sys.argv[1], int(sys.argv[2]), sys.argv[3]
    run_calibration(path_csv, idx, path_output)
  else:
    path_csv, path_output = sys.argv[1], sys.argv[3]
    for idx in sys.argv[2].split(','):
      run_calibration(path_csv, idx, path_output)